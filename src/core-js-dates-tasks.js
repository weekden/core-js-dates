...
function getNextFriday(date) {
  ...
  if (currentDay > friday) {
    addDayCoin = friday - currentDay + week;
  }
  if (currentDay < friday) {
    addDayCoin = friday - currentDay;
  }

  data.setDate(data.getDate() + addDayCoin);
  return data;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const firstPoint = new Date(dateStart);
  const secondPoint = new Date(dateEnd);
  const between = secondPoint - firstPoint;
  return between / 24 / 1000 / 3600 + 1;
}
...
function getCountWeekendsInMonth(month, year) {
  let weekEndsDay = 0;
  const coinAllDaysInMounth = new Date(year, month, 0).getDate();

  for (let i = 1; i <= coinAllDaysInMounth; i += 1) {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const result = [];
  const startData = new Date(period.start.split('-').reverse().join('-'));
  const endData = new Date(period.end.split('-').reverse().join('-'));
  const currentDay = new Date(startData);
  let counterWorksDays = 0;
  let isWorkDay = true;
  while (currentDay <= endData) {
    if (isWorkDay) {
      const formattedDate = new Date(currentDay)
        .toLocaleDateString('ru')
        .replaceAll('.', '-');
      result.push(formattedDate);
      counterWorksDays += 1;
      if (counterWorksDays === countWorkDays) {
        isWorkDay = false;
        counterWorksDays = 0;
      }
